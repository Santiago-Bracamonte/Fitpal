...
def modificar_rutina_y_ejercicios(id_rutina, nombre=None, descripcion=None, duracion=None, nuevos_ejercicios_ids=None):
    if not rutina_dao.obtener_rutina_por_id(id_rutina):
        print("Rutina a modificar no encontrada.")
        return False
        
    rutina_obj = Rutina(id_rutina, nombre, descripcion, duracion) 
    if rutina_dao.actualizar_rutina(rutina_obj):
        if nuevos_ejercicios_ids is not None:
            ejercicios_actuales = rutina_ejercicio_dao.obtener_ejercicios_de_rutina(id_rutina)
            for ej in ejercicios_actuales:
                rutina_ejercicio_dao.eliminar_asociaciones_por_ejercicio(ej.id_ejercicio)
            
            for ejercicio_id in nuevos_ejercicios_ids:
                rutina_ejercicio_dao.asociar_ejercicio_a_rutina(id_rutina, ejercicio_id)
        return True
    return False
